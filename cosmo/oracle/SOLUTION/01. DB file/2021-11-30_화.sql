...
select eno, ename, salary, nvl(commission, 0) as commission
...
SELECT ENAME, SALARY, SALARY + 300
  FROM EMPLOYEE;
--2 사원의 이름， 급여， 연간총수입을 총수입이 많은 것부터 작은 순으로 출력하시오. 
...
-- 6. 1981 년 2월 20일 부터 1981 년 5월 1일 사이에 입시한사원의 이름， 담당 업무， 입사일을 출력하시오. 
SELECT ENAME, JOB, HIREDATE
  FROM EMPLOYEE
 WHERE HIREDATE BETWEEN '81/02/20' AND '81/05/01';
-- 7. 부서 번호가 20 및 30에 속한 사원의 이름과 부서번호를 출력하도| 
-- 이름을 기준{내림치순)으로 영문자순으로 출력하시오 
SELECT ENAME, DNO
  FROM EMPLOYEE
 WHERE DNO IN (20, 30)
 ORDER BY ENAME DESC;
...
SELECT ENO, ENAME, SALARY + NVL(COMMISSION, 0) FROM EMPLOYEE WHERE COMMISSION IS NULL;

--SELECT ENO, ENAME, SALARY + COMMISSION FROM EMPLOYEE WHERE COMMISSION IS NULL;
